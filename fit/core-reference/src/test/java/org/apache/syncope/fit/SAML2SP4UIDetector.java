...
public class SAML2SP4UIDetector {

    private static final Logger LOG = LoggerFactory.getLogger(SAML2SP4UIDetector.class);

    private static Boolean ENABLED;

    public static boolean isSAML2SP4UIAvailable() {
        synchronized (LOG) {
            if (ENABLED == null) {
                try {
                    new SyncopeClientFactoryBean().
                            ...
                            getService(SAML2SP4UIService.class).getMetadata("http://localhost:9080/syncope", "saml2sp");
                    ENABLED = true;
                } catch (Exception e) {
                    // ignore
                    ENABLED = false;
