...
public class LiveSyncValidator extends AbstractValidator<LiveSyncCheck, LiveSync> {
    ...
    public boolean isValid(final LiveSync resource, final ConstraintValidatorContext context) {
        ...
        if (objectClasses.size() < resource.getProvisions().size()) {
            context.buildConstraintViolationWithTemplate(getTemplate(EntityViolationType.InvalidResource,
                            "Each livesync provision  requires a different" + ObjectClass.class.getSimpleName())).
                    addPropertyNode("provisions").addConstraintViolation();
            return false;
        }
