...
public class WAOIDCJWKSGeneratorService implements OidcJsonWebKeystoreGeneratorService {
    ...
    public JsonWebKeySet store(final JsonWebKeySet jsonWebKeySet) {
        OIDCOPService service = waRestClient.getService(OIDCOPService.class);
        OIDCOPTO oidcOPTO = new OIDCOPTO();
        oidcOPTO.setJWKS(jsonWebKeySet.toJson(JsonWebKey.OutputControlLevel.INCLUDE_PRIVATE));
        service.set(oidcOPTO);
    ...
    public Resource generate() {
        ...
        Resource result = new ByteArrayResource(oidcOPTO.getJWKS().getBytes(StandardCharsets.UTF_8), "OIDC JWKS");
