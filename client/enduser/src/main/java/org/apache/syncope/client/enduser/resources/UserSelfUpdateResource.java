    ...
    protected ResourceResponse newResourceResponse(final IResource.Attributes attributes) {
        ...
        try {
            ...
            if (UserRequestValidator.compliant(userTO, customForm, false)) {
                ...
                UserPatch diffPatch =
                        AnyOperations.diff(userTO,
                                SyncopeEnduserSession.get().getSelfTO(), false);

                Set<AttrPatch> patchPlainAttrs = diffPatch.getPlainAttrs();
                Set<AttrTO> patchVirtualAttrs = diffPatch.getVirAttrs();
                final CustomAttributesInfo formPlainAttrsInfo = customForm.get(SchemaType.PLAIN.name());
                final CustomAttributesInfo formVirtualAttrsInfo = customForm.get(SchemaType.VIRTUAL.name());

                // clean patch to avoid unwanted deletions of hidden schemas (custom form)
                if (formPlainAttrsInfo != null) {
                    if (!formPlainAttrsInfo.getShow()) {
                        patchPlainAttrs.clear();
                    } else {
                        CollectionUtils.filter(patchPlainAttrs, new Predicate<AttrPatch>() {

                            @Override
                            public boolean evaluate(final AttrPatch patchPlainAttr) {
                                return formPlainAttrsInfo.getAttributes().
                                        containsKey(patchPlainAttr.getAttrTO().getSchema());
                            }
                        });
                    }
                }
                if (formVirtualAttrsInfo != null) {
                    if (!formVirtualAttrsInfo.getShow()) {
                        patchVirtualAttrs.clear();
                    } else {
                        CollectionUtils.filter(patchVirtualAttrs, new Predicate<AttrTO>() {

                            @Override
                            public boolean evaluate(final AttrTO patchVirtualAttr) {
                                return formVirtualAttrsInfo.getAttributes().
                                        containsKey(patchVirtualAttr.getSchema());
                            }
                        });
                    }
                }
