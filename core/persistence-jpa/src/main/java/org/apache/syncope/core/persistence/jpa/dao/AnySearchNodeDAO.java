 ...
 */
public class AnySearchNodeDAO extends AbstractAnySearchDAO {
    ...
    protected static void visitLeaf(
            final AnySearchNode.Leaf node,
            final Set<SearchSupport.SearchView> from,
            final List<String> where) {

        from.add(node.getFrom());
        where.add(node.getClause());
    }
    ...
    protected static void visitNode(
        ...
        node.asLeaf().ifPresentOrElse(
                leaf -> visitLeaf(leaf, from, where),
                () -> {
        ...
        switch (cond.getType()) {
            ...
            case LIKE:
                if (schema.getType() == AttrSchemaType.String || schema.getType() == AttrSchemaType.Enum) {
                    ...
                    return new AnySearchNode.Leaf(svs.field(), "1=2");
                }
                break;
