...
[WARNING]
The value of the `security.aesSecretKey` property in the `core.properties` file is used for AES-based encryption /
decryption: besides password values, this is also used whenever reversible encryption is needed, throughout the whole
system. +
...
https://docs.spring.io/spring-framework/reference/7.0/integration/scheduling.html#scheduling-cron-expression[cron^] expression describing how
