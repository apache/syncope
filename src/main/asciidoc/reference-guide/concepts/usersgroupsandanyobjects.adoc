...
. `mfa` (Users only) - encrypted value representing the enrolled MFA device to use during the authentication process,
when <<authentiation-credentials,MFA is enabled>>;
