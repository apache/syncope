...
[TIP]
The <<swagger,Swagger extension>> might be greatly helpful when working with RESTful services.

==== Client Library

The Java client library is meant for simplifying the interaction with the <<core>> by hiding the underlying HTTP
...
You need first to build an instance of `SyncopeClientFactoryBean` by providing the deployment base URL, as follows:
...
<<apache-camel-provisioning-engine>> provides the
...
<3> get users in the root realm with no values for `loginDate`, e.g. that have never authenticated to the
given deployment
